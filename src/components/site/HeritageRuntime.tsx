...
// Runtime for the Heritage template: scroll-reveal, count-up stats, sticky
// bar hairline, and the mobile menu toggle. Motion-light by design.
export default function HeritageRuntime() {
  useEffect(() => {
    ...
    // 1) Reveal on scroll.
    const revealEls = Array.from(root.querySelectorAll<HTMLElement>(".hr-reveal"));
    ...
    // 3) Sticky bar.
    const top = root.querySelector<HTMLElement>(".hr-top");
    const onScroll = () => top?.classList.toggle("solid", window.scrollY > 24);
    onScroll();
    window.addEventListener("scroll", onScroll, { passive: true });

    // 4) Mobile menu.
    const menuBtn = root.querySelector<HTMLElement>(".hr-menu");
    const toggle = () => root.classList.toggle("nav-open");
    const close = () => root.classList.remove("nav-open");
    menuBtn?.addEventListener("click", toggle);
    const navLinks = Array.from(root.querySelectorAll<HTMLElement>(".hr-nav a"));
