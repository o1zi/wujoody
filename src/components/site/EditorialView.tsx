...
// Editorial empty image state: warm paper + faint accent grid + line icon.
function Slot({ src, label, kind = "image" }: { src: string | null; label: string; kind?: "person" | "image" }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: "#ECE7DB" }}>
      <div style={{ position: "absolute", inset: 0, opacity: 0.16, backgroundImage: "linear-gradient(var(--accent) 1px, transparent 1px), linear-gradient(90deg, var(--accent) 1px, transparent 1px)", backgroundSize: "22px 22px" }} />
      <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" style={{ position: "relative", opacity: 0.8 }}>
        <path d={icon} />
      </svg>
    </div>
  );
...
function EdSocial({ href, label, children }: { href: string; label: string; children: ReactNode }) {
  return (
...
export default function EditorialView({ content, slug, caps }: { content: SiteContent; slug: string; caps: PlanCaps }) {
  ...
  const Brand = (
    <>
      {brand.ar}
      <span style={{ fontFamily: "var(--serif)", fontStyle: "italic", color: "var(--accent)" }}>.</span>
    </>
  );
  ...
  return (
    ...
    >
      ...
      <header className="ed-top">
        ...
        <button className="ed-menu" aria-label="القائمة"><span /><span /><span /></button>
      </header>
      ...
      <main id="top">
        ...
        <section className="ed-hero">
          <div className="ed-wrap">
            <div className="ed-hero-kicker ed-reveal">{content.hero.eyebrow}</div>
            <h1 className="ed-hero-title ed-reveal" data-d="1">{Brand}</h1>
            <div className="ed-hero-grid">
              <div>
                <p className="ed-hero-sub ed-reveal" data-d="2">{content.hero.subtitle}</p>
                <div className="ed-scrollcue ed-reveal" data-d="3"><span className="ln" /> مرّر للأسفل · SCROLL</div>
              </div>
              <div className="ed-hero-meta ed-reveal" data-d="2">
          ...
          <section className="ed-sec" id="about">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                ...
                <span className="ed-kicker">من نحن · About</span>
                <span className="stem" />
              </div>
              <div className="ed-about">
                <div className="ed-reveal" data-d="1">
                  <h2 className="ed-about-lead">{content.about.lead}</h2>
                  <p className="ed-about-body">{content.about.body}</p>
                </div>
                <div className="ed-side ed-reveal" data-d="2">
        ...
        {show("credentials") && (content.credentials.badges.length > 0 || content.credentials.clients.length > 0) && (
          <section className="ed-sec" id="credentials">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">★</span>
                <span className="ed-kicker">الاعتمادات والثقة · Credentials</span>
                <span className="stem" />
              </div>
              {content.credentials.lead ? <p className="ed-lead ed-reveal" data-d="1" style={{ marginBottom: 30 }}>{content.credentials.lead}</p> : null}
              {content.credentials.badges.length > 0 && (
                <div className="ed-cred ed-reveal" data-d="2">
              ...
              {content.credentials.clients.length > 0 && (
                <div className="ed-clients ed-reveal" data-d="3">
                  {content.credentials.clients.map((cl, i) => (
                    <div className="ed-client" key={i} title={cl.name}>
                      {cl.logo ? (
                        // eslint-disable-next-line @next/next/no-img-element
        ...
        {/* SERVICES */}
        {show("services") && (
          <section className="ed-sec" id="services">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">02</span>
                <span className="ed-kicker">الخدمات · Services</span>
                <span className="stem" />
              </div>
              <h2 className="ed-title ed-reveal" data-d="1" style={{ marginBottom: 16 }}>{content.services.title}</h2>
              <p className="ed-lead ed-reveal" data-d="2" style={{ marginBottom: 40 }}>{content.services.lead}</p>
              <div className="ed-svc-grid">
                {content.services.items.map((s, i) => (
                  <div className="ed-svc ed-reveal" data-d={(i % 2) + 1} key={i}>
        ...
        {/* STATS */}
        {show("stats") && (
          <section className="ed-sec" id="stats">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">03</span>
                <span className="ed-kicker">بالأرقام · By the numbers</span>
                <span className="stem" />
              </div>
              <div className="ed-stats">
                {content.stats.map((st, i) => {
                  ...
                  return (
                    <div className="ed-stat ed-reveal" data-d={(i % 4) + 1} key={i}>
                      <div className="num">
        ...
        {/* PROCESS */}
        {show("process") && (
          <section className="ed-sec" id="process">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">04</span>
                <span className="ed-kicker">منهجية العمل · Process</span>
                <span className="stem" />
              </div>
              <div className="ed-proc">
                {content.process.map((p, i) => (
                  <div className="ed-step ed-reveal" data-d={(i % 4) + 1} key={i}>
        ...
        {/* TEAM */}
        {show("team") && (
          <section className="ed-sec" id="team">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">06</span>
                <span className="ed-kicker">الفريق · Leadership</span>
                <span className="stem" />
              </div>
              <h2 className="ed-title ed-reveal" data-d="1" style={{ marginBottom: 40 }}>عقولٌ تقف خلف كل عمل.</h2>
              <div className="ed-team">
                {content.team.items.map((mb, i) => (
                  <div className="ed-member ed-reveal" data-d={(i % 4) + 1} key={i}>
                    <div className="ph">
                      <span className="n">{String(i + 1).padStart(2, "0")}</span>
                      <Slot src={mb.image} label={mb.name} kind="person" />
                    </div>
                    <h3>{mb.name}</h3>
        ...
        {show("testimonials") && content.testimonials.length > 0 && (
          <section className="ed-sec" id="voices">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">07</span>
                <span className="ed-kicker">آراء العملاء · Clients</span>
                <span className="stem" />
              </div>
              <div className="ed-quotes">
                {content.testimonials.map((t, i) => (
                  <div className="ed-quote ed-reveal" data-d={(i % 3) + 1} key={i}>
                    <p><span className="mk">«</span>{t.quote}<span className="mk">»</span></p>
                    <div className="by"><span className="ln" /><b>{t.name}</b><span>{t.role}</span></div>
                  </div>
                ))}
              </div>
            </div>
        ...
        {show("faq") && content.faq.items.length > 0 && (
          <section className="ed-sec" id="faq">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">08</span>
                <span className="ed-kicker">الأسئلة الشائعة · FAQ</span>
                <span className="stem" />
              </div>
              <div className="ed-faq ed-reveal" data-d="1">
        ...
        {/* BOOKING */}
        {show("booking") && (
          <section className="ed-sec" id="booking">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">09</span>
                <span className="ed-kicker">احجز استشارة · Book</span>
                <span className="stem" />
              </div>
              <h2 className="ed-title ed-reveal" data-d="1" style={{ marginBottom: 14 }}>احجز استشارتك الأولى</h2>
              <p className="ed-lead ed-reveal" data-d="2" style={{ marginBottom: 34 }}>اختر الوقت المناسب لك وسيتواصل معك أحد مهندسينا لتأكيد الموعد.</p>
              <div className="ed-reveal" data-d="2" style={{ maxWidth: 640 }}>
        ...
        {show("calculator") && content.calculator.types.some((t) => t.name) && (
          <section className="ed-sec" id="calculator">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">~</span>
                <span className="ed-kicker">حاسبة التكلفة · Estimate</span>
                <span className="stem" />
              </div>
              {content.calculator.lead ? <p className="ed-lead ed-reveal" data-d="1" style={{ marginBottom: 30 }}>{content.calculator.lead}</p> : null}
              <div className="ed-reveal" data-d="2">
        ...
        {/* CONTACT */}
        {show("contact") && (
          <section className="ed-sec" id="contact">
            <div className="ed-wrap">
              <div className="ed-head ed-reveal">
                <span className="ed-index">10</span>
                <span className="ed-kicker">تواصل معنا · Contact</span>
                <span className="stem" />
              </div>
              <div className="ed-contact">
                <div className="ed-reveal" data-d="1">
                  <h2 className="ed-title" style={{ fontSize: "clamp(28px,3.6vw,46px)", marginBottom: 16 }}>لديك مشروع؟<br />لنضع له أساساً راسخاً.</h2>
                  <p className="ed-lead" style={{ marginBottom: 34 }}>أخبرنا عن فكرتك، وسيتواصل معك أحد مهندسينا خلال يوم عمل واحد.</p>
                  <ContactForm slug={slug} waNumber={caps.whatsapp && waNumber.length >= 8 ? waNumber : ""} brand={brand.ar} />
                </div>
                <div className="ed-cinfo ed-reveal" data-d="2">
              ...
              {content.contact.mapQuery ? (
                <div className="ed-map ed-reveal" data-d="2">
                  <iframe
                    title="موقع المكتب"
        ...
        <div className="ed-wrap">
          <div className="ed-foot-grid">
            ...
            <div className="col">
              ...
              <a href="#contact">{content.contact.email}</a>
              <a href="#contact">{content.contact.phone}</a>
              <a href="#contact">{content.contact.office}</a>
            </div>
          </div>
