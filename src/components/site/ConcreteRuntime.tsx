...
// Runtime for the Concrete template: scroll-reveal, count-up stats, sticky bar,
// mobile menu. Texture and form-tie details are pure CSS.
export default function ConcreteRuntime() {
  useEffect(() => {
    const root = document.querySelector<HTMLElement>(".cn");
    if (!root) return;
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    const revealEls = Array.from(root.querySelectorAll<HTMLElement>(".cn-reveal"));
    ...
    const top = root.querySelector<HTMLElement>(".cn-top");
    const onScroll = () => top?.classList.toggle("solid", window.scrollY > 24);
    onScroll();
    window.addEventListener("scroll", onScroll, { passive: true });

    const menuBtn = root.querySelector<HTMLElement>(".cn-menu");
    const toggle = () => root.classList.toggle("nav-open");
    const close = () => root.classList.remove("nav-open");
    menuBtn?.addEventListener("click", toggle);
    const navLinks = Array.from(root.querySelectorAll<HTMLElement>(".cn-nav a"));
