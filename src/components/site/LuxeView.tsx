...
const ACCENTS: Record<string, { hex: string; rgb: string }> = {
  ...
  azure: { hex: "#7FA8E0", rgb: "127,168,224" },
  sage: { hex: "#A6BE86", rgb: "166,190,134" },
};
...
function Slot({ src, label, kind = "image" }: { src: string | null; label: string; kind?: "person" | "image" }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: "#161A22" }}>
      <div style={{ position: "absolute", inset: 0, opacity: 0.12, backgroundImage: "linear-gradient(var(--accent) 1px, transparent 1px), linear-gradient(90deg, var(--accent) 1px, transparent 1px)", backgroundSize: "22px 22px" }} />
      <div style={{ position: "absolute", width: 180, height: 180, borderRadius: "50%", background: "radial-gradient(circle, rgba(var(--accent-rgb),.2), transparent 70%)" }} />
      <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="1.2" strokeLinecap="round" strokeLinejoin="round" style={{ position: "relative", opacity: 0.85 }}>
        <path d={icon} />
      </svg>
    </div>
  );
...
function LxSocial({ href, label, children }: { href: string; label: string; children: ReactNode }) {
  return (
...
export default function LuxeView({ content, slug, caps }: { content: SiteContent; slug: string; caps: PlanCaps }) {
  ...
  const head = (kicker: string) => (
    <div className="lx-head lx-reveal">
      <span className="lx-kicker"><span className="ln" />{kicker}<span className="ln" /></span>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <header className="lx-top">
        ...
        <button className="lx-menu" aria-label="القائمة"><span /><span /><span /></button>
      </header>
      ...
      <div className="lx-content">
        <main id="top">
          ...
          <section className="lx-hero">
            <div className="lx-wrap">
              <div className="lx-hero-kicker lx-reveal">{content.hero.eyebrow}</div>
              <h1 className="lx-hero-title lx-reveal" data-d="1">{Brand}</h1>
              <div className="lx-hero-divider lx-reveal" data-d="2" />
              <p className="lx-hero-sub lx-reveal" data-d="2">{content.hero.subtitle}</p>
              <div className="lx-hero-meta lx-reveal" data-d="3">
              ...
              <div className="lx-scrollcue lx-reveal" data-d="4"><span className="m" />SCROLL</div>
            </div>
          </section>
            ...
            <section className="lx-sec" id="about">
              <div className="lx-wrap">
                ...
                <div className="lx-about">
                  <div className="lx-reveal" data-d="1">
                    <h2 className="lx-about-lead">{content.about.lead}</h2>
                    <p className="lx-about-body">{content.about.body}</p>
                  </div>
                  <div className="lx-side lx-reveal" data-d="2">
          ...
          {show("credentials") && (content.credentials.badges.length > 0 || content.credentials.clients.length > 0) && (
            <section className="lx-sec" id="credentials">
              <div className="lx-wrap">
                {head("الاعتمادات والثقة")}
                {content.credentials.lead ? <p className="lx-lead lx-reveal" data-d="1" style={{ marginBottom: 34 }}>{content.credentials.lead}</p> : null}
                {content.credentials.badges.length > 0 && (
                  <div className="lx-cred lx-reveal" data-d="2">
                ...
                {content.credentials.clients.length > 0 && (
                  <div className="lx-clients lx-reveal" data-d="3">
                    {content.credentials.clients.map((cl, i) => (
                      <div className="lx-client" key={i} title={cl.name}>
                        {cl.logo ? (
                          // eslint-disable-next-line @next/next/no-img-element
          ...
          {show("services") && (
            <section className="lx-sec" id="services">
              <div className="lx-wrap">
                {head("الخدمات")}
                <h2 className="lx-title lx-reveal" data-d="1" style={{ textAlign: "center", marginBottom: 12 }}>{content.services.title}</h2>
                <p className="lx-lead lx-reveal" data-d="2" style={{ textAlign: "center", marginBottom: 48 }}>{content.services.lead}</p>
                <div className="lx-svc-grid">
                  {content.services.items.map((s, i) => (
                    <div className="lx-svc lx-reveal" data-d={(i % 3) + 1} key={i}>
          ...
          {/* STATS */}
          {show("stats") && (
            <section className="lx-sec" id="stats">
              <div className="lx-wrap">
                {head("بالأرقام")}
                <div className="lx-stats">
                  {content.stats.map((st, i) => {
                    ...
                    return (
                      <div className="lx-stat lx-reveal" data-d={(i % 4) + 1} key={i}>
                        <div className="num">
          ...
          {/* PROCESS */}
          {show("process") && (
            <section className="lx-sec" id="process">
              <div className="lx-wrap">
                {head("منهجية العمل")}
                <div className="lx-proc">
                  {content.process.map((p, i) => (
                    <div className="lx-step lx-reveal" data-d={(i % 4) + 1} key={i}>
          ...
          {/* TEAM */}
          {show("team") && (
            <section className="lx-sec" id="team">
              <div className="lx-wrap">
                {head("الفريق")}
                <h2 className="lx-title lx-reveal" data-d="1" style={{ textAlign: "center", marginBottom: 48 }}>عقولٌ تقف خلف كل عمل.</h2>
                <div className="lx-team">
                  {content.team.items.map((mb, i) => (
                    <div className="lx-member lx-reveal" data-d={(i % 4) + 1} key={i}>
          ...
          {show("testimonials") && content.testimonials.length > 0 && (
            <section className="lx-sec" id="voices">
              <div className="lx-wrap">
                {head("آراء العملاء")}
                <div className="lx-quotes lx-reveal" data-d="1">
                  {content.testimonials.map((t, i) => (
                    <div className={`lx-quote ${i === 0 ? "on" : ""}`} key={i}>
                  ...
                  {content.testimonials.length > 1 && (
                    <div className="lx-qnav">
                      <button data-q="prev" aria-label="السابق">→</button>
                      <button data-q="next" aria-label="التالي">←</button>
                    </div>
                  )}
                </div>
              </div>
          ...
          {show("faq") && content.faq.items.length > 0 && (
            <section className="lx-sec" id="faq">
              <div className="lx-wrap">
                {head("الأسئلة الشائعة")}
                <div className="lx-faq lx-reveal" data-d="1">
          ...
          {/* BOOKING */}
          {show("booking") && (
            <section className="lx-sec" id="booking">
              <div className="lx-wrap">
                {head("احجز استشارة")}
                <h2 className="lx-title lx-reveal" data-d="1" style={{ textAlign: "center", marginBottom: 12 }}>احجز استشارتك الأولى</h2>
                <p className="lx-lead lx-reveal" data-d="2" style={{ textAlign: "center", marginBottom: 40 }}>اختر الوقت المناسب لك وسيتواصل معك أحد مهندسينا لتأكيد الموعد.</p>
                <div className="lx-contact-card lx-reveal" data-d="2" style={{ maxWidth: 660, margin: "0 auto" }}>
          ...
          {show("calculator") && content.calculator.types.some((t) => t.name) && (
            <section className="lx-sec" id="calculator">
              <div className="lx-wrap">
                {head("حاسبة التكلفة")}
                {content.calculator.lead ? <p className="lx-lead lx-reveal" data-d="1" style={{ textAlign: "center", marginBottom: 36 }}>{content.calculator.lead}</p> : null}
                <div className="lx-contact-card lx-reveal" data-d="2" style={{ maxWidth: 900, margin: "0 auto" }}>
          ...
          {show("contact") && (
            <section className="lx-sec" id="contact">
              <div className="lx-wrap">
                {head("تواصل معنا")}
                <h2 className="lx-title lx-reveal" data-d="1" style={{ textAlign: "center", marginBottom: 48 }}>لديك مشروع؟ لنبدأه معاً.</h2>
                <div className="lx-contact">
                  <div className="lx-contact-card lx-reveal" data-d="1">
                    <ContactForm slug={slug} waNumber={caps.whatsapp && waNumber.length >= 8 ? waNumber : ""} brand={brand.ar} />
                  </div>
                  <div className="lx-contact-card lx-cinfo lx-reveal" data-d="2">
                ...
                {content.contact.mapQuery ? (
                  <div className="lx-map lx-reveal" data-d="2">
                    <iframe
                      title="موقع المكتب"
                      src={`https://www.google.com/maps?q=${encodeURIComponent(content.contact.mapQuery)}&z=15&output=embed`}
                      width="100%"
                      height="320"
                      style={{ border: 0, display: "block", filter: "invert(0.9) hue-rotate(180deg) brightness(0.95) contrast(0.9) saturate(0.8)" }}
                      loading="lazy"
        ...
        <footer className="lx-foot">
          <div className="lx-wrap">
            <div className="big lx-reveal">{Brand}</div>
            <p className="tag lx-reveal" data-d="1">{content.hero.subtitle}</p>
            <div className="lx-foot-nav lx-reveal" data-d="1">
              {show("about") && <a href="#about">من نحن</a>}
              {show("services") && <a href="#services">الخدمات</a>}
              {show("projects") && <a href="#projects">المشاريع</a>}
              {show("team") && <a href="#team">الفريق</a>}
