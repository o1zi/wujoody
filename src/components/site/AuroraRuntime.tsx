...
// Runtime for the Aurora template: scroll-reveal, count-up stats, sticky bar,
// mobile menu. The aurora mesh is pure CSS.
export default function AuroraRuntime() {
  useEffect(() => {
    const root = document.querySelector<HTMLElement>(".au");
    if (!root) return;
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    const revealEls = Array.from(root.querySelectorAll<HTMLElement>(".au-reveal"));
    ...
    const top = root.querySelector<HTMLElement>(".au-top");
    const onScroll = () => top?.classList.toggle("solid", window.scrollY > 24);
    onScroll();
    window.addEventListener("scroll", onScroll, { passive: true });

    const menuBtn = root.querySelector<HTMLElement>(".au-menu");
    const toggle = () => root.classList.toggle("nav-open");
    const close = () => root.classList.remove("nav-open");
    menuBtn?.addEventListener("click", toggle);
    const navLinks = Array.from(root.querySelectorAll<HTMLElement>(".au-nav a"));
