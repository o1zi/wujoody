...
// Concrete palette — strong industrial accents on warm grey concrete.
const ACCENTS: Record<string, { hex: string; rgb: string }> = {
  bronze: { hex: "#B5462A", rgb: "181,70,42" }, // burnt sienna / rust (default)
  terracotta: { hex: "#B5462A", rgb: "181,70,42" },
  azure: { hex: "#2E5D72", rgb: "46,93,114" }, // steel
  sage: { hex: "#5E6B3E", rgb: "94,107,62" }, // olive
};
...
function Slot({ src, label, kind = "image" }: { src: string | null; label: string; kind?: "person" | "image" }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: "#BEBAB0" }}>
      ...
      <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" style={{ position: "relative", opacity: 0.85 }}>
        <path d={icon} />
      </svg>
    </div>
  );
...
function CnSocial({ href, label, children }: { href: string; label: string; children: ReactNode }) {
  return (
...
export default function ConcreteView({ content, slug, caps }: { content: SiteContent; slug: string; caps: PlanCaps }) {
  ...
  const head = (ix: string, kicker: string) => (
    <div className="cn-head cn-reveal">
  ...
  return (
    ...
    >
      ...
      <header className="cn-top">
        ...
        <button className="cn-menu" aria-label="القائمة"><span /><span /><span /></button>
      </header>
      ...
      <div className="cn-content">
        <main id="top">
          ...
          <section className="cn-hero">
            <div className="cn-wrap" style={{ width: "100%" }}>
              <div className="cn-hero-kicker cn-reveal">{content.hero.eyebrow}</div>
              <h1 className="cn-hero-title cn-reveal" data-d="1">{Brand}</h1>
              <div className="cn-hero-row cn-reveal" data-d="2">
                <p className="cn-hero-sub">{content.hero.subtitle}</p>
                <div className="cn-hero-meta cn-tie">
              ...
              <div className="cn-scrollcue cn-reveal" data-d="3"><span className="sq" />مرّر للأسفل · SCROLL</div>
            </div>
          </section>
              ...
              <div className="cn-wrap">
                ...
                <div className="cn-about">
                  <div className="cn-reveal" data-d="1">
                    ...
                    <p className="cn-about-body">{content.about.body}</p>
                  </div>
                  <div className="cn-side cn-tie cn-reveal" data-d="2">
              ...
              <div className="cn-wrap">
                ...
                <div className="cn-svc-grid">
                  {content.services.items.map((s, i) => (
                    <div className="cn-svc cn-reveal" data-d={(i % 3) + 1} key={i}>
          ...
          {show("credentials") && (content.credentials.badges.length > 0 || content.credentials.clients.length > 0) && (
            <section className="cn-sec" id="credentials">
              <div className="cn-wrap">
                {head("★", "الاعتمادات · Credentials")}
                {content.credentials.lead ? <p className="cn-lead cn-reveal" data-d="1" style={{ marginBottom: 28 }}>{content.credentials.lead}</p> : null}
                {content.credentials.badges.length > 0 && (
                  <div className="cn-cred cn-tie cn-reveal" data-d="2">
                ...
                {content.credentials.clients.length > 0 && (
                  <div className="cn-clients cn-reveal" data-d="3">
                    {content.credentials.clients.map((cl, i) => (
                      <div className="cn-client" key={i} title={cl.name}>
                        {cl.logo ? (
                          // eslint-disable-next-line @next/next/no-img-element
          ...
          {/* TEAM */}
          {show("team") && (
            <section className="cn-sec" id="team">
              <div className="cn-wrap">
                {head("06", "الفريق · Team")}
                <h2 className="cn-title cn-reveal" data-d="1" style={{ marginBottom: 40 }}>العقول.</h2>
                <div className="cn-team">
                  {content.team.items.map((mb, i) => (
                    <div className="cn-member cn-reveal" data-d={(i % 4) + 1} key={i}>
          ...
          {show("testimonials") && content.testimonials.length > 0 && (
            <section className="cn-sec" id="voices">
              <div className="cn-wrap">
                {head("07", "آراء العملاء · Clients")}
                {content.testimonials.slice(0, 1).map((t, i) => (
                  <div className="cn-quote cn-reveal" data-d="1" key={i}>
                ...
                {content.testimonials.length > 1 && (
                  <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(260px,1fr))", gap: "clamp(18px,2.5vw,30px)", marginTop: "clamp(34px,5vw,54px)" }}>
                    {content.testimonials.slice(1).map((t, i) => (
                      <div className="cn-reveal" data-d={(i % 3) + 1} key={i} style={{ borderTop: "3px solid var(--ink)", paddingTop: 18 }}>
                        <p style={{ fontSize: 16.5, lineHeight: 1.6, color: "var(--ink)" }}>«{t.quote}»</p>
          ...
          {show("faq") && content.faq.items.length > 0 && (
            <section className="cn-sec" id="faq">
              <div className="cn-wrap">
                {head("08", "الأسئلة الشائعة · FAQ")}
                <div className="cn-faq cn-reveal" data-d="1">
          ...
          {/* BOOKING */}
          {show("booking") && (
            <section className="cn-sec" id="booking">
              <div className="cn-wrap">
                {head("09", "احجز استشارة · Book")}
                <h2 className="cn-title cn-reveal" data-d="1" style={{ marginBottom: 14 }}>احجز استشارتك.</h2>
                <p className="cn-lead cn-reveal" data-d="2" style={{ marginBottom: 40 }}>اختر الوقت المناسب لك وسيتواصل معك أحد مهندسينا لتأكيد الموعد.</p>
                <div className="cn-reveal" data-d="2" style={{ maxWidth: 660 }}>
          ...
          {show("calculator") && content.calculator.types.some((t) => t.name) && (
            <section className="cn-sec" id="calculator">
              <div className="cn-wrap">
                {head("~", "حاسبة التكلفة · Estimate")}
                {content.calculator.lead ? <p className="cn-lead cn-reveal" data-d="1" style={{ marginBottom: 36 }}>{content.calculator.lead}</p> : null}
                <div className="cn-reveal" data-d="2">
          ...
          {/* CONTACT */}
          {show("contact") && (
            <section className="cn-sec" id="contact">
              <div className="cn-wrap">
                {head("10", "تواصل معنا · Contact")}
                <h2 className="cn-title cn-reveal" data-d="1" style={{ marginBottom: 40 }}>لديك مشروع؟ لنبنِه.</h2>
                <div className="cn-contact cn-tie cn-reveal" data-d="1">
                  <div>
                    <ContactForm slug={slug} waNumber={caps.whatsapp && waNumber.length >= 8 ? waNumber : ""} brand={brand.ar} />
                  </div>
                  <div className="cn-cinfo">
                ...
                {content.contact.mapQuery ? (
                  <div className="cn-map cn-reveal" data-d="2">
                    <iframe
                      title="موقع المكتب"
                      src={`https://www.google.com/maps?q=${encodeURIComponent(content.contact.mapQuery)}&z=15&output=embed`}
                      width="100%"
                      height="320"
                      style={{ border: 0, display: "block", filter: "grayscale(0.5) contrast(1.05)" }}
                      loading="lazy"
        ...
        <footer className="cn-foot">
          <div className="cn-wrap">
            <div className="big cn-reveal">{Brand}</div>
            <div className="cn-foot-row">
              <p className="tag">{content.hero.subtitle}</p>
              <div className="cn-foot-nav">
                {show("about") && <a href="#about">من نحن</a>}
                {show("services") && <a href="#services">الخدمات</a>}
                {show("projects") && <a href="#projects">المشاريع</a>}
                {show("team") && <a href="#team">الفريق</a>}
