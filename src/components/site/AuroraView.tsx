...
const ACCENTS: Record<string, { hex: string; rgb: string }> = {
  bronze: { hex: "#6D5DF6", rgb: "109,93,246" }, // indigo-violet (default)
  terracotta: { hex: "#F0686A", rgb: "240,104,106" }, // coral
  azure: { hex: "#3B82F6", rgb: "59,130,246" }, // blue
  sage: { hex: "#10B981", rgb: "16,185,129" }, // emerald
};
...
function Slot({ src, label, kind = "image" }: { src: string | null; label: string; kind?: "person" | "image" }) {
  ...
  return (
    <div style={{ position: "absolute", inset: 0, display: "flex", alignItems: "center", justifyContent: "center", overflow: "hidden", background: "rgba(255,255,255,.45)" }}>
      <div style={{ position: "absolute", width: 160, height: 160, borderRadius: "50%", background: "radial-gradient(circle, rgba(var(--accent-rgb),.28), transparent 70%)" }} />
      <svg width="44" height="44" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" style={{ position: "relative", opacity: 0.9 }}>
        <path d={icon} />
      </svg>
    </div>
  );
...
function AuSocial({ href, label, children }: { href: string; label: string; children: ReactNode }) {
  return (
...
export default function AuroraView({ content, slug, caps }: { content: SiteContent; slug: string; caps: PlanCaps }) {
  ...
  const head = (label: string) => (
    <div className="au-head au-reveal">
      <span className="au-pill">{label}</span>
    </div>
  );
  ...
  return (
    ...
    >
      ...
      <header className="au-top">
        <div className="au-top-inner">
          <a className="au-brand" href="#top">
            {brand.logo ? (
              // eslint-disable-next-line @next/next/no-img-element
          ...
          <button className="au-menu" aria-label="القائمة"><span /><span /><span /></button>
        </div>
      </header>
      ...
      <div className="au-content">
        <main id="top">
          ...
          <section className="au-hero">
            <div className="au-wrap au-hero-inner">
              <span className="au-hero-badge au-reveal"><span className="dot" />{content.hero.eyebrow}</span>
              <h1 className="au-hero-title au-reveal" data-d="1">{Brand}</h1>
              <p className="au-hero-sub au-reveal" data-d="2">{content.hero.subtitle}</p>
              <div className="au-hero-meta au-reveal" data-d="3">
              ...
              <div className="au-scrollcue au-reveal" data-d="4"><span className="m" />SCROLL</div>
            </div>
          </section>
          ...
          {show("about") && (
            <section className="au-sec" id="about">
              <div className="au-wrap">
                {head("من نحن · About")}
                <div className="au-card au-reveal" data-d="1">
                  <div className="au-about">
                    <div>
                      <h2 className="au-about-lead">{content.about.lead}</h2>
                      <p className="au-about-body">{content.about.body}</p>
                    </div>
          ...
          {/* SERVICES */}
          {show("services") && (
            <section className="au-sec" id="services">
              <div className="au-wrap">
                {head("الخدمات · Services")}
                <h2 className="au-title au-reveal" data-d="1" style={{ marginBottom: 12 }}>{content.services.title}</h2>
                <p className="au-lead au-reveal" data-d="2" style={{ marginBottom: 38 }}>{content.services.lead}</p>
                <div className="au-svc-grid">
                  {content.services.items.map((s, i) => (
                    <div className="au-svc au-reveal" data-d={(i % 3) + 1} key={i}>
          ...
          {show("credentials") && (content.credentials.badges.length > 0 || content.credentials.clients.length > 0) && (
            <section className="au-sec" id="credentials">
              <div className="au-wrap">
                {head("الاعتمادات · Credentials")}
                {content.credentials.lead ? <p className="au-lead au-reveal" data-d="1" style={{ marginBottom: 28 }}>{content.credentials.lead}</p> : null}
                {content.credentials.badges.length > 0 && (
                  <div className="au-cred au-reveal" data-d="2">
                ...
                {content.credentials.clients.length > 0 && (
                  <div className="au-clients au-reveal" data-d="3">
                    {content.credentials.clients.map((cl, i) => (
                      <div className="au-client" key={i} title={cl.name}>
                        {cl.logo ? (
                          // eslint-disable-next-line @next/next/no-img-element
          ...
          {/* STATS */}
          {show("stats") && (
            <section className="au-sec" id="stats">
              <div className="au-wrap">
                ...
                <div className="au-stats">
                  {content.stats.map((st, i) => {
                    ...
                    return (
                      <div className="au-stat au-reveal" data-d={(i % 4) + 1} key={i}>
                        <div className="num">
          ...
          {/* PROCESS */}
          {show("process") && (
            <section className="au-sec" id="process">
              <div className="au-wrap">
                {head("منهجية العمل · Process")}
                <div className="au-proc">
                  {content.process.map((p, i) => (
                    <div className="au-step au-reveal" data-d={(i % 4) + 1} key={i}>
          ...
          {/* TEAM */}
          {show("team") && (
            <section className="au-sec" id="team">
              <div className="au-wrap">
                {head("الفريق · Team")}
                <h2 className="au-title au-reveal" data-d="1" style={{ marginBottom: 38 }}>الفريق.</h2>
                <div className="au-team">
                  {content.team.items.map((mb, i) => (
                    <div className="au-member au-reveal" data-d={(i % 4) + 1} key={i}>
          ...
          {show("testimonials") && content.testimonials.length > 0 && (
            <section className="au-sec" id="voices">
              <div className="au-wrap">
                {head("آراء العملاء · Clients")}
                <div className="au-quotes">
                  {content.testimonials.map((t, i) => (
                    <div className="au-quote au-reveal" data-d={(i % 3) + 1} key={i}>
          ...
          {show("faq") && content.faq.items.length > 0 && (
            <section className="au-sec" id="faq">
              <div className="au-wrap">
                {head("الأسئلة الشائعة · FAQ")}
                <div className="au-faq au-reveal" data-d="1">
          ...
          {/* BOOKING */}
          {show("booking") && (
            <section className="au-sec" id="booking">
              <div className="au-wrap">
                {head("احجز استشارة · Book")}
                <div className="au-card au-reveal" data-d="1" style={{ maxWidth: 700, margin: "0 auto" }}>
                  <h2 className="au-title" style={{ fontSize: "clamp(24px,3vw,38px)", marginBottom: 10 }}>احجز استشارتك الأولى</h2>
                  <p className="au-lead" style={{ marginBottom: 30 }}>اختر الوقت المناسب لك وسيتواصل معك أحد مهندسينا لتأكيد الموعد.</p>
          ...
          {show("calculator") && content.calculator.types.some((t) => t.name) && (
            <section className="au-sec" id="calculator">
              <div className="au-wrap">
                {head("حاسبة التكلفة · Estimate")}
                <div className="au-card au-reveal" data-d="1">
                  {content.calculator.lead ? <p className="au-lead" style={{ marginBottom: 26 }}>{content.calculator.lead}</p> : null}
          ...
          {/* CONTACT */}
          {show("contact") && (
            <section className="au-sec" id="contact">
              <div className="au-wrap">
                {head("تواصل معنا · Contact")}
                <h2 className="au-title au-reveal" data-d="1" style={{ marginBottom: 38 }}>لديك مشروع؟ لنبدأه.</h2>
                <div className="au-contact">
                  <div className="au-card au-reveal" data-d="1">
                    <ContactForm slug={slug} waNumber={caps.whatsapp && waNumber.length >= 8 ? waNumber : ""} brand={brand.ar} />
                  </div>
                  <div className="au-card au-cinfo au-reveal" data-d="2">
                ...
                {content.contact.mapQuery ? (
                  <div className="au-map au-reveal" data-d="2">
                    <iframe
                      title="موقع المكتب"
        ...
        <footer className="au-foot">
          <div className="au-wrap">
            <div className="big au-reveal">{Brand}</div>
            <p className="tag au-reveal" data-d="1">{content.hero.subtitle}</p>
            <div className="au-foot-nav au-reveal" data-d="1">
              {show("about") && <a href="#about">من نحن</a>}
              {show("services") && <a href="#services">الخدمات</a>}
              {show("projects") && <a href="#projects">المشاريع</a>}
              {show("team") && <a href="#team">الفريق</a>}
