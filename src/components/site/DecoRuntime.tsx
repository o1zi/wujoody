...
// Runtime for the Deco template: scroll-reveal, count-up stats, sticky bar,
// mobile menu. Ornaments and sunburst are pure CSS.
export default function DecoRuntime() {
  useEffect(() => {
    const root = document.querySelector<HTMLElement>(".dc");
    if (!root) return;
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    const revealEls = Array.from(root.querySelectorAll<HTMLElement>(".dc-reveal"));
    ...
    const top = root.querySelector<HTMLElement>(".dc-top");
    const onScroll = () => top?.classList.toggle("solid", window.scrollY > 24);
    onScroll();
    window.addEventListener("scroll", onScroll, { passive: true });

    const menuBtn = root.querySelector<HTMLElement>(".dc-menu");
    const toggle = () => root.classList.toggle("nav-open");
    const close = () => root.classList.remove("nav-open");
    menuBtn?.addEventListener("click", toggle);
    const navLinks = Array.from(root.querySelectorAll<HTMLElement>(".dc-nav a"));
