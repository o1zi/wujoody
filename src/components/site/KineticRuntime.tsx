...
// Runtime for the Kinetic template: scroll-reveal, count-up stats, sticky bar,
// mobile menu. The marquee tickers and rotating seal are pure CSS.
export default function KineticRuntime() {
  useEffect(() => {
    const root = document.querySelector<HTMLElement>(".kn");
    if (!root) return;
    const reduce = window.matchMedia("(prefers-reduced-motion: reduce)").matches;

    const revealEls = Array.from(root.querySelectorAll<HTMLElement>(".kn-reveal"));
    ...
    const top = root.querySelector<HTMLElement>(".kn-top");
    const onScroll = () => top?.classList.toggle("solid", window.scrollY > 24);
    onScroll();
    window.addEventListener("scroll", onScroll, { passive: true });

    const menuBtn = root.querySelector<HTMLElement>(".kn-menu");
    const toggle = () => root.classList.toggle("nav-open");
    const close = () => root.classList.remove("nav-open");
    menuBtn?.addEventListener("click", toggle);
    const navLinks = Array.from(root.querySelectorAll<HTMLElement>(".kn-nav a"));
